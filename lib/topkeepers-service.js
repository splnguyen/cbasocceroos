...
const PAGE_CONCURRENCY = 6;     // bounded parallel page fetches (stay friendly to the API)
const MIN_APPEARANCES  = 1;     // fairness guard: ignore keepers who haven't played
const CACHE_TTL_MS  = 10 * 60 * 1000; // the GK table moves slowly; sweep is expensive
...
async function sweepPlayers(league, season, apiKey) {
  const first = await apiGet('/players', { league, season, page: 1 }, apiKey);
  ...
  async function worker() {
    while (nextPage <= totalPages) {
      const page = nextPage++;
      const json = await apiGet('/players', { league, season, page }, apiKey);
  ...
  return { players: all, capped: reportedPages > MAX_PAGES };
}
...
async function fetchTopKeepers(query = {}) {
  ...
  const { players, capped } = await sweepPlayers(league, season, apiKey);

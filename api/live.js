...
const { fetchLive, getLastRateLimit } = require('../lib/match-service');

loadEnvFiles();

// Expose the latest api-football rate-limit snapshot (per-minute + per-day) on
// the response headers and in the JSON body.
...
module.exports = async (req, res) => {
  ...
  try {
    const data = await fetchLive(req.query || {});
    ...
    return res.status(status).json(withRateLimit(res, { ok: false, error: err.message || 'Failed to load live data' }));
  }
};
